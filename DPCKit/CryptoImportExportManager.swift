...
//
// Borrowed from here: https://github.com/DigitalLeaves/CryptoExportImportManager/blob/master/CryptoLoadExternalCertificate/CryptoExportImportManager.swift
import Foundation
...
//        var resultType: SecTrustResultType = SecTrustResultType(rawValue: UInt32(0))! // result will be ignored.
        var error: CFError?
        let evaluateStatus = SecTrustEvaluateWithError(secTrust!, &error)
        if let error = error {
            debugPrint("Trust evaluation error", error)
            return nil
        }
